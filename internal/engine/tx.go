...
// TODO 每个操作的执行单元就是一个事务，哪怕它是单条语句 隐式事务

// 用于记录事务读取的键范围（start 到 stop），后续用来做读写冲突检测（Conflict Detection）
...
// KV transaction
type KVTX struct {
	// a read-only snapshot, 数据库的快照（只读）
	snapshot BTree
...
func (kv *KV) Begin(tx *KVTX) {
	...
	// 给这个事务设置一个析构钩子，如果事务对象被 GC 了，但 tx.done = false，就会触发断言失败。这是一个安全检查机制，防止忘记 Commit 或 Abort
