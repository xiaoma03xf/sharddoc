...
func newBTree() *BTree {
	leaf := newLeafNode(nil)        // 创建一个空的叶子节点
	r := newInteriorNode(nil, leaf) // 创建一个内部节点，child 指向 leaf
	leaf.p = r                      // 设置 leaf 的父节点是 r
	return &BTree{
		root:     r,
		first:    leaf,
		leaf:     1,
...
func (bt *BTree) Insert(key int, value string) {
	...
	// leaf 是分裂后的原始叶子节点（保留前半部分键值对）
	// 	- 将`midNode`设置为原始叶子节点
	// - 更新父节点中的子节点指针，让它指向分裂后的新节点`leaf.next`
	// - 设置新节点`leaf.next`的父节点为`p`

	var midNode node
	midNode = leaf

	// 原索引 oldIndex 指向 leaf.next
	p.kcs[oldIndex].child = leaf.next
	leaf.next.setParent(p)

	// 	interior：当前处理的内部节点，初始为叶子节点的父节点 p。
	// interiorP：interior 的父节点（可能为 nil，如果是根节点的父节点）。
	// 这两个变量用于递归处理内部节点的分裂，从叶子节点的父节点开始向上传播。
